...
function MODE:FigureOutConsequences()
	...
	for _, v in ipairs(self.saved.Players or {}) do
		...
		data[ply].now = {}
		data[ply].now.name = self.saved.DeathNames[ply] or (ply:GetNWString("PlayerName") or ply:Name()) or "Nieznany"
...
function MODE:SetupTraitor(ply)
	...
	zb.GiveRole(ply, "Operatywnik", Color(121, 5, 69))
	ply:SetPlayerClass()
...
function MODE:SetupSurvivor(ply)
	...
	zb.GiveRole(ply, "Ocalały", Color(230, 74, 74))
	ply:SetPlayerClass()
...
function MODE:RoundStart()
	...
	// --- GLIDE FUEL SYSTEM MODIFICATION ---
	
	// Włączamy system paliwa w Glide
	local glideFuel = GetConVar("glide_fuel")
	if glideFuel then
		glideFuel:SetBool(true)
	end

	// Ustawiamy mnożnik zużycia paliwa.
	// Wartość 1.0 to standard. Ustawienie 4.0 sprawi, że paliwo zniknie 4x szybciej.
	// Daje to około 4-5 minut intensywnej jazdy na pełnym baku.
	local glideFuelMult = GetConVar("glide_fuel_mult")
	if glideFuelMult then
		glideFuelMult:SetFloat(4.0) 
	else
		// Fallback dla niektórych wersji Glide, które używają innej nazwy zmiennej
		local glideCons = GetConVar("glide_fuel_consumption")
		if glideCons then
			glideCons:SetFloat(4.0)
		end
	end
...
function MODE:CanPlayerSuicide(ply)
	if IsValid(ply) and ply:IsPlayer() then
		ply:ChatPrint("Nie ma tak łatwo!!!")
		return false
	end
end
...
function MODE:SpawnDeltaSquad(count)
	...
	for i, ply in RandomPairs(player.GetAll()) do
		...
		zb.GiveRole(ply, "Oddział Delta", Color(79, 10, 10))
	...
	net.Start("zb_commandertransmit")
		net.WriteString("Nie zapomnijcie: jesteście tu, aby ewakuować ocalałych z tego obszaru. Nawet nie próbujcie wracać do bazy bez przynajmniej jednego z nich.")
...
function MODE:SpawnSquadHelicopter()
	...
	if #heliPoints > 0 then
		self:CreateTimer("UWUCopter", math.random(240, 300), 1, function()
			...
			if #squad == 0 then
				self:BroadcastCommander("Przykro mi, ewakuacja nie przybędzie. Jesteście skazani na siebie. Lepiej ze sobą skończcie.")
			else
				self:BroadcastCommander("Helikopter ewakuacyjny jest w waszej okolicy. Znajdźcie go i spróbujcie nadać jakiś sygnał.")
...
function MODE:InitiateCQExtraction()
	...
	if spawnPoint then
		self:CreateTimer("CQExtract", math.random(120, 180), 1, function()
			...
			if #squad == 0 then
				self:BroadcastCommander("Przykro mi, ewakuacja nie przybędzie. Jesteście zgubieni.")
			else
				self:BroadcastCommander("Oddział Delta jest na miejscu. Znajdźcie ich i ruszajcie za nimi do punktu ewakuacji.")
...
function MODE:CanPlayerEnterVehicle(ply, ent) -- damdn i forgot about the broken mode hooks lmao
	if ply.PlayerClassName == "furry" then
		if (ply.cantdrivecd or 0) < CurTime() then
			ply:Notify("Yyy, nie mam pojęcia jak się prowadzi.", 0, "idkdrive", 0)
...
local killfurries = {
	"Jeden dziwoląg mniej.",
	"Oczyszczone.",
	"Giń, ścierwo!",
	"To nawet nie był człowiek!",
	"Wściekła bestia uciszona.",
	"Przeklęte stworzenie.",
	"Giń! Giń! Giń!",
	"Nienawidzę tych rzeczy.",
	"Widzimy się w piekle.",
	"Wymierzam sprawiedliwość.",
	"Jedna abominacja martwa."
}
...
function MODE:SpawnGlideHelicopter()
		...
		if extracted then
			for _, v in ipairs(players) do
				...
				if !self.saved.ContractorEscapee then
					...
					self:BroadcastContractor("Ktoś właśnie uciekł! Misja zakończona porażką! Twój kontrakt zostaje rozwiązany.")
				end
			end
			...
			timer.Simple(5, function()
				self:BroadcastCommander("Helikopter ewakuacyjny właśnie opuścił obszar. Przykro mi stary, chyba zostałeś sam.")
			end)
		end
	end
	...
	heli:CallOnRemove("CommanderReply", function()
		...
		timer.Simple(5, function()
			self:BroadcastCommander("Straciłem kontakt z helikopterem... Szlag...")
		end)
	end)
