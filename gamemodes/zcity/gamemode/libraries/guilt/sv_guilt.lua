...
hook.Add("HomigradDamage", "GuiltReg", function(ply, dmgInfo, hitgroup, ent, harm) 
	...
	-- Ile karmy miałoby zejść w tym ticku (oryginalna logika)
	local factor = math.max(((1 - (zb.GuiltTable[Victim][Attacker] or 0)) / 1), 0)
	local karmaLoss = add * factor

	-- LIMIT: maksymalnie 20 karmy straty za jedną ofiarę (łącznie w rundzie)
	local cap = zb.MaxKarmaLossPerKill or 20
	local prevLoss = zb.HarmDoneKarma[Victim][Attacker] or 0
	local allowed = math.max(0, cap - prevLoss)

	karmaLoss = math.min(karmaLoss, allowed)

	-- Zastosuj stratę karmy
	Attacker.Karma = math.Clamp((Attacker.Karma or 100) - karmaLoss, -60, zb.MaxKarma)

	-- Zapisz realną stratę (żeby forgive oddawało poprawnie i żeby limit działał)
	zb.HarmDoneKarma[Victim][Attacker] = prevLoss + karmaLoss
