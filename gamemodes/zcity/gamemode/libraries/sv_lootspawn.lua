...
local loot_boxes = {
	...
	["models/props_generic/crate_36x36.mdl"] = {8,"all"},
	["models/props_generic/crate_48x48.mdl"] = {8,"all"},
	["models/props_junk/storage_crate01.mdl"] = {10,"weapons"},
...
function hg.GenerateLoot(ply,ent,func)
    ...
    if not tab then
        ...
        if curRound.name == "pathowogen" then
            -- Jeśli tabele istnieją, użyj ich, w przeciwnym razie użyj domyślnych broni
            if chances and chances[1] and chances[1][2] then
                tab = chances[1][2]
            else
                -- Awaryjna tabela, gdyby Z-City nie miało configu dla PathOwOgen
                tab = {"weapon_glock18", "weapon_knife", "med_band_small"} 
            end
        end
    end
...
hook.Add("PostCleanupMap", "addboxs", function()
    ...
    timer.Simple(.5,function()
        ...
        timer.Create("SpawnTheBoxes", 6, 0, function() hook_Run("Boxes Think") end)
    end)
end)
...
hook.Add("Boxes Think", "SpawnBoxes", function()
    -- Sprawdzanie rundy i PathOwOgen
    if zb.ROUND_STATE ~= 1 or (not CurrentRound().LootSpawn and CurrentRound().name ~= "pathowogen") then return end

    -- ==============================================================
    -- >>> LIMIT PROPOW <<<
    -- ==============================================================
    local currentProps = 0
    for _, ent in ipairs(ents.GetAll()) do
        if ent.IsSpawned then -- Liczymy tylko te stworzone przez skrypt
            currentProps = currentProps + 1
        end
    end

    -- Jeśli jest już 40 lub więcej, nie spawnuj kolejnego
    if currentProps >= 40 then return end
    -- ==============================================================

    -- Kod odpowiedzialny za szukanie gracza i spawnowanie lootu obok niego
    ...
    if (math.random(100) <= 70) and not CurrentRound().noBoxes then
